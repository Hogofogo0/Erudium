...
public class Compressor extends BlockWithEntity implements BlockEntityProvider {
    ...
    private static VoxelShape SHAPE = Block.createCuboidShape(0,0,0,16,27,16);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            NamedScreenHandlerFactory screenHandlerFactory = ((CompressorEntity) world.getBlockEntity(pos));
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.COMPRESSOR_ENTITY, (world1, pos, state1, blockEntity) -> blockEntity.tick(world1,pos, state1));
    }
